    <tr class="text-center">
        <td class="bg-blue-300 border-solid border-2 border-gray-300" colspan="2">Nom</td><td class="bg-blue-300 border-solid border-gray-300">Categorie</td>
    </tr>
    <?php
    $selectp = $db -> prepare("select * FROM produit order by libellePro");
    $selectp -> execute();
    ...
    for($i=0;$i<$nbr;$i++){
        ...
        <tr>
            ...
            <td class="border-solid border-2 border-gray-300" colspan="2"><?=$fetch[$i]["libellePro"]?> (<?=$fetch[$i]["unite"]?>)</td>
            <td class="border-solid border-2 text-center border-gray-300"> <?=$fetch[$i]["codeCa"]?></td>
            <td class="border-solid border-2 border-gray-300">
            <a href="../produit/sup.php?code=<?=$fetch[$i]['codePro']?>">
